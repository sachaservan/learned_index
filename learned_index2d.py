...
if __name__ == "__main__": 
    ...
    y_max = 1450000
    parser = OptionParser()
    parser.add_option("-f", "--file", dest="file", type="str", default="data_2d.csv", help="data file")
    ...
    parser.add_option("-g", "--gran", dest="gran", type="float", default=1, help="granularity (in percent) of each bin")
    ...
    for i in range(len(bins) - 1):
        lower = bins[i]
        upper = bins[i + 1]
        pred_lower, pred_upper, y_ranges = predict_2D(lower, upper, y_min, y_max, w, d, node_x, nodes_y)
